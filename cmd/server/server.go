...
func main() {
	...
	homieClient := homie.NewClient(config.Prefix(), config.Host(), config.Port(), config.MQTTPrefix(), config.Ssl(), config.SSLConfig().CA, config.SSLConfig().ClientCert, config.SSLConfig().Privkey, config.HomieName(), "weatherStation")
